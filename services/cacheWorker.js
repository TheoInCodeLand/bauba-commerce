...
async function refreshHomepageCache() {
    console.log('--retry-- [CacheWorker] Refreshing homepage cache...');
    try {
        ...
        console.log(`--success-- [CacheWorker] Homepage cache refreshed at ${new Date().toISOString()}`);
    } catch (err) {
...
function startCacheWorker() {
    ...
    console.log('--launched-- [CacheWorker] Homepage cache worker started (refresh every 5 min)');
}
